 ...
 *
 * @author Dan DiTursi modified by Justin Montagne
 * @version 11 February 2023
 */
public class GameState
{
    // instance variables - replace the example below with your own
    private ArrayList<Card> cells; // Numbered from 0-3; empty cells will always belast
    ...
    public GameState resultState(ArrayList<Action> Alist) {
        ...
        result.listOfActions.addAll(Alist);
        return result;
    }
    ...
    */
     ...
     */
    public boolean stateEquals(GameState gs){
        
        //Check if numCellsFree is the same
        if(numCellsFree != gs.numCellsFree){
            return false;
        }

        //Check if the cells have the same cards
        for (int i = 0; i < cells.size(); i++) {
            if(!cells.get(i).equals(gs.cells.get(i))){
                return false;
            }
        }

        //Check if the size of the foundations is the same
        for(int i = 1; i < foundations.length; i++){
            if(foundations[i] != gs.foundations[i]){
                return false;
            }
        }

        //Check if the top card of each pile is the same
        for (int i = 0; i < 8; i++) {
            //Check if one is empty while the other isnt 
            if(tableau.get(i).isEmpty() && !gs.tableau.get(i).isEmpty())
                return false;
            ...
            if(!tableau.get(i).isEmpty() && !gs.tableau.get(i).isEmpty()){
                if(!tableau.get(i).get(tableau.get(i).size()-1).equals(gs.tableau.get(i).get(gs.tableau.get(i).size()-1))){
                    return false;
                }
            }
    ...
    /*
     * Find the total score of the current GameState. Score is based off of the number of cards on top of a card we are looking at and the number
     * of cards needed on the foundation before the card we are looking at. This will find the minimum amount of moves needed to win the game.
     */
    public int h(){
        int score = 0;
        ...
        for(int i = 0; i < cells.size(); i++){
            if(cells.get(i) == null){
                ...
                int s = cells.get(i).getSuit();
                int foundation = foundations[s];
                //We will see how many cards are needed before this one on the foundation 
                //(ex. we have a 3 of hearts on top of foundation, our card is a 6 of hearts, means we need
                // the 4 and 5 of hearts first which will add 2 to the score)
                score += cells.get(i).getRank() - foundation; 
            }
        }
        
        //Go through all cards and find the score of them and add to total
        for(ArrayList<Card> a : tableau){
            for(Card c : a){
                    score += getScore(c);
            }
        ...
        //Adding 1 to the score as we need to make a move
        return score + 1;
    }

    /*
     * Calculate the score of the current card in the tableau
     * Looking for the tableau the card is in and then finds how many cards are on top of it by starting from the top
     * and adding 1 until we find the card
     */
    public int getScore(Card card){
        int score = 0, cardsAbove = 0, index = 0;
        boolean found = false;
        ArrayList<Card> tableauFound = null;
        

        //Get how many cards we need before this one (same as the check above for the cells)
        int s = card.getSuit();
        int foundation = foundations[s];
        score += card.getRank() - foundation;


        //Find the card (to find the tableau)
        while(index < tableau.size() && !found){
            ArrayList<Card> a = tableau.get(index);
            index++;
            for(Card c : a){
                if (c.equals(card)){
                    found = true;
        ...
        //Find how many cards are above the card
       
        for(int i = tableauFound.size()-1; i >=0; i--){
            
            if(tableauFound.get(i).equals(card)){
                break;
            }
        ...
        score += cardsAbove; //Add that to the score
